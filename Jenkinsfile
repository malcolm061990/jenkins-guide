...
// Performing Software Tests
    stage('TEST') {
      parallel {
        ...
        stage('Quality Tests') {
          steps {
            sh 'docker login --username malcolm666 --password adgjmptw1'
            sh 'docker tag nodeapp-dev:trunk malcolm666/nodeapp-dev:latest'
            sh 'docker push malcolm666/nodeapp-dev:latest'
          }
        }
      }
      post {
            ...
            steps {
                    retry(3) {
                        timeout(time:10, unit: 'MINUTES') {
                            sh 'docker tag nodeapp-dev:trunk malcolm666/nodeapp-prod:latest'
                            sh 'docker push malcolm666/nodeapp-prod:latest'
                            sh 'docker save malcolm666/nodeapp-prod:latest | gzip > nodeapp-prod-golden.tar.gz'
                        }
                    }
